...
interface SchemaOrgValidationResult {
  aligned: boolean;
  issues: ValidationIssue[];
  warnings: ValidationWarning[];
  score: number;
  alignmentDetails: { matches: number; total: number };
}

interface CrossRegistryValidationResult {
  compatible: boolean;
  issues: ValidationIssue[];
  warnings: ValidationWarning[];
  score: number;
  registryCompatibility: {
    difCompatible: boolean;
    cheqdCompatible: boolean;
    openVerifiableCompatible: boolean;
  };
}
...
interface ValidationResult {
  ...
  warnings: ValidationWarning[];
  details: {
    w3cCompliance?: W3CValidationResult;
    schemaOrgAlignment?: SchemaOrgValidationResult;
    crossRegistryCompatibility?: CrossRegistryValidationResult;
    productionReadiness?: ProductionReadinessResult;
    securityCompliance?: SecurityValidationResult;
  };
}
...
interface ValidationReport {
  ...
  categoryScores: {
    w3cCompliance: number;
    schemaOrgAlignment: number;
    crossRegistryCompatibility: number;
...
class EnhancedSchemaValidator {
   ...
   */
  async validateW3CCompliance(schema: JSONSchema, schemaPath: string): Promise<W3CValidationResult> {
    ...
    try {
      ...
      if (schema.properties && schema.properties['@context']) {
        ...
        if (!contextDef.items || !contextDef.items.enum) {
          ...
          const hasStandardContext = contextDef.items.enum.some((ctx: string) =>
   ...
   */
  async validateSchemaOrgAlignment(schema: JSONSchema, schemaPath: string): Promise<SchemaOrgValidationResult> {
    const issues: ValidationIssue[] = [];
    const warnings: ValidationWarning[] = [];
    ...
    try {
      ...
      if (hasSchemaOrgEquivalent) {
        ...
        if (schema.properties && schema.properties['@context']) {
          ...
          const hasSchemaOrgContext = contextDef.items && contextDef.items.enum && 
            contextDef.items.enum.some((ctx: string) => ctx.includes('schema.org'));
      ...
      return { aligned: false, issues, warnings, score: 0, alignmentDetails: { matches: 0, total: 0 } };
    }
  }
   ...
   */
  async validateCrossRegistryCompatibility(schema: JSONSchema, schemaPath: string): Promise<CrossRegistryValidationResult> {
    const issues: ValidationIssue[] = [];
    const warnings: ValidationWarning[] = [];
    ...
    try {
      ...
      if (schema.properties && schema.properties['@context']) {
        const contextDef = schema.properties['@context'];
        if (contextDef.items && contextDef.items.enum) {
          const hasCheqdContext = contextDef.items.enum.some((ctx: string) => 
   ...
   */
  async validateProductionReadiness(schema: JSONSchema, schemaPath: string): Promise<ProductionReadinessResult> {
    const issues: ValidationIssue[] = [];
    const warnings: ValidationWarning[] = [];
    ...
    try {
      ...
      readinessChecks.hasExamples = !!schema.examples || 
        (schema.properties && Object.values(schema.properties).some((prop: any) => prop.examples));
      ...
      if (schema.properties) {
        const hasValidationConstraints = Object.values(schema.properties).some((prop: any) =>
      ...
      Object.entries(readinessChecks).forEach(([check, passed]) => {
        if (!passed) {
          const severity: 'error' | 'warning' = ['hasTitle', 'hasDescription', 'hasRequiredFields'].includes(check) ? 
            'error' : 'warning';
          ...
          }[check as keyof typeof readinessChecks];
      ...
      return { ready: false, issues, warnings, score: 0, readinessChecks };
    }
  }
   ...
   */
  async validateSecurityCompliance(schema: JSONSchema, schemaPath: string): Promise<SecurityValidationResult> {
    const issues: ValidationIssue[] = [];
    const warnings: ValidationWarning[] = [];
    ...
    try {
      ...
      return { secure: false, issues, warnings, score: 0, securityChecks };
    }
  }
   ...
   */
  async validateAllSchemas(directories: string[] = ['schemas/v1', 'drafts']): Promise<ValidationResult[]> {
   ...
   */
  generateRecommendations(results: ValidationResult[]): Array<{
    type: string;
    priority: 'high' | 'medium' | 'low';
    message: string;
    affectedCount?: number;
...
export { EnhancedSchemaValidator, main }; 