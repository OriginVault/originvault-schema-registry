...
exports.createDerivative = exports.getProvenanceChain = exports.verifyManifest = exports.getProvenance = exports.addProvenance = exports.createManifest = void 0;
...
const createManifest = async (req, res) => {
    try {
        const { content_hash, title, creator, creation_tool, creation_params, assertions = [] } = req.body;
...
const addProvenance = async (req, res) => {
    try {
        const { content_hash, action, actor, tool, parameters, input_hashes, output_hashes } = req.body;
        ...
        if (!record) {
            ...
            record = {
                ...
                contentHash: content_hash,
                manifestHash: crypto_1.default.createHash('sha256').update(JSON.stringify(manifest)).digest('hex'),
                timestamp,
                provenance: [],
        ...
        record.manifestHash = crypto_1.default
            .createHash('sha256')
...
exports.addProvenance = addProvenance;
// GET /api/c2pa/provenance/:hash - Get provenance history
const getProvenance = async (req, res) => {
    try {
...
const verifyManifest = async (req, res) => {
    try {
...
const getProvenanceChain = async (req, res) => {
    try {
        const { hash } = req.params;
...
exports.getProvenanceChain = getProvenanceChain;
// POST /api/c2pa/create-derivative - Create derivative content record
const createDerivative = async (req, res) => {
    try {
        const { source_hash, derivative_hash, transformation, actor, tool, parameters } = req.body;
...
// Helper functions
async function verifyProvenance(record) {
    return {
...
async function buildProvenanceChain(hash) {
    const chain = [];
    const visited = new Set();
    async function traverse(currentHash) {
        ...
        const ingredientAssertion = record.c2paManifest.assertions.find(a => a.label === 'c2pa.ingredient');
...
async function verifyProvenanceChain(chain) {
    return {
        valid: true,
        chain_length: chain.length,
